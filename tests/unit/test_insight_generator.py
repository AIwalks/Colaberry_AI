...
def test_no_kpis_no_fingerprints_returns_empty_insights():
    result = InsightGenerator().generate_insights([], [])
...
def test_high_confidence_kpi_produces_insight():
    result = InsightGenerator().generate_insights([make_kpi(confidence=0.8)], [])
    assert len(result.insights) == 1


def test_kpi_confidence_above_threshold_produces_kpi_insight_type():
    result = InsightGenerator().generate_insights([make_kpi(confidence=0.9)], [])
...
def test_kpi_confidence_exactly_0_7_does_not_trigger():
    ...
    result = InsightGenerator().generate_insights([make_kpi(confidence=0.7)], [])
    assert result.insights == []


def test_kpi_confidence_below_threshold_produces_no_insight():
    result = InsightGenerator().generate_insights([make_kpi(confidence=0.5)], [])
    assert result.insights == []


def test_kpi_confidence_zero_produces_no_insight():
    result = InsightGenerator().generate_insights([make_kpi(confidence=0.0)], [])
    assert result.insights == []
...
def test_high_risk_fingerprint_produces_insight():
    result = InsightGenerator().generate_insights([], [make_fingerprint(risk_level="high")])
    assert len(result.insights) == 1
...
def test_medium_risk_fingerprint_produces_no_insight():
    result = InsightGenerator().generate_insights([], [make_fingerprint(risk_level="medium")])
    assert result.insights == []


def test_low_risk_fingerprint_produces_no_insight():
    result = InsightGenerator().generate_insights([], [make_fingerprint(risk_level="low")])
    assert result.insights == []
...
def test_multiple_qualifying_kpis_produce_multiple_insights():
    ...
    result = InsightGenerator().generate_insights(kpis, [])
...
def test_both_qualifying_kpi_and_fingerprint_produce_two_insights():
    ...
    result = InsightGenerator().generate_insights(kpis, fingerprints)
...
def test_multiple_high_risk_fingerprints_produce_multiple_insights():
    ...
    result = InsightGenerator().generate_insights([], fps)
    assert len(result.insights) == 2
...
def test_generated_count_matches_insights_list_length():
    ...
    result = InsightGenerator().generate_insights(kpis, fps)
...
def test_kpi_insight_title_contains_kpi_name():
    result = InsightGenerator().generate_insights([make_kpi("avg_logins", confidence=0.8)], [])
...
def test_kpi_insight_confidence_matches_kpi_confidence():
    result = InsightGenerator().generate_insights([make_kpi(confidence=0.85)], [])
...
def test_risk_insight_title_contains_pattern_name():
    result = InsightGenerator().generate_insights([], [make_fingerprint(pattern_name="disengagement")])
    assert "disengagement" in result.insights[0].title


def test_risk_insight_body_contains_entity_id():
    result = InsightGenerator().generate_insights([], [make_fingerprint(entity_id="student_42")])
    assert "student_42" in result.insights[0].body


def test_risk_insight_confidence_matches_fingerprint_score():
    result = InsightGenerator().generate_insights([], [make_fingerprint(score=0.95)])
    assert result.insights[0].confidence == 0.95


def test_risk_insight_source_patterns_contains_pattern_and_score():
    result = InsightGenerator().generate_insights([], [make_fingerprint(pattern_name="disengagement", score=0.9)])
...
def test_kpi_missing_kpi_name_uses_unknown_fallback():
    ...
    result = InsightGenerator().generate_insights([kpi], [])
...
def test_fingerprint_missing_risk_level_produces_no_insight():
    ...
    result = InsightGenerator().generate_insights([], [fp])
    assert result.insights == []
