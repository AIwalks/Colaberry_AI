...
The endpoint applies deterministic mapping rules. `DbTriggerProcessingService` reads rules from `AI_ChatBot_TriggerRules`, evaluates them against `AI_ChatBot_TriggerData`, writes a row to `AI_ChatBot_TriggeredUsers`, and logs to `AI_ChatBot_EngagementEvents` and `AI_ChatBot_AuditLog`.
...
The service maps `(trigger_type, trigger_level)` to a planned action list using fixed rules in `_ACTION_MAP`.

### Stub TriggerType values (unit tests only)
| `trigger_type` | `actions_planned` |
|---|---|
| `"nudge_needed"` | `["queue_nudge_message"]` |
| `"progress_milestone"` | `["queue_congrats_message"]` |

### Real TriggerType values from `AI_ChatBot_TriggerRules`
| `trigger_type` | `actions_planned` (Low / High / Unknown level) |
|---|---|
| `"All"` | `["queue_outbound_message"]` |
| `"Capstone"` | `["queue_outbound_message"]` |
| `"InClass"` | `["queue_outbound_message"]` |
| `"InterviewPast4Wks"` | `["queue_outbound_message"]` |
| `"InterviewPrep"` | `["queue_outbound_message"]` |
| `"IPBC"` | `["queue_outbound_message"]` |
| `"PostInterview"` | `["queue_outbound_message"]` |
| any unrecognised value | `[]` (rule not found → `accepted: false`) |

Level `"None"` (KPI within threshold range) produces no action for any type.
...
- No external messaging (Twilio, Mandrill, voice) — `queue_outbound_message` is a planned action only; delivery happens via the trigger worker and `OutboundDeliveryService`
